...
3. **PRE-RENDER GATE — `python3 timing/check-edl.py <clip-dir>`** (NO render — instant). Reads the source: master view-timeline vs each beat's first-content time. Predicts blank-left and names any late-firing Mode-A beat. **Fix here, before wasting a render.** Must print `PASS`.
4. **RENDER GATE — `python3 timing/check-render.py <clip-dir>`** (after a draft render). The authoritative backstop on the actual pixels (also catches dark-rendering graphics the static check can't see). Must print `PASS`.
5. **Human spot-check** at the BEAT-START of every Mode-A beat and every view transition (where blanks hide) — NOT mid-beat payoffs.
6. **HQ render** only after both gates are green.
6. **Commit** (renders gitignored).
